...
//             <button type="button" className="btn btn-outline-light m-1"><Link to={`/subjects/subject-update/${row.original._id}`} className="card-link m-2">Edit</Link></button>
...
function List() {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: "Department",
        accessorFn: (row) => row.department?.name || "Unknown", // Access department name
        id: "departmentName",
      },
      { header: "Active", accessorKey: "active" },
      {
        ...
        cell: ({ row }) => (
          <div style={{ textAlign: "center" }}>
            <button type="button" className="btn btn-outline-light m-1">
              <Link to={`/subjects/subject-update/${row.original._id}`} className="card-link m-2">
                Edit
              </Link>
            </button>
            <button
            ...
            >
              <a href="#" style={{ textColor: "blue" }}>Delete</a>
            </button>
          </div>
        ),
      },
